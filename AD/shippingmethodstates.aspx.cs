...
// $Header: /v6.2/Web/Admin/shippingmethodstates.aspx.cs 4     9/03/06 8:39p Redwoodtree $
// ------------------------------------------------------------------------------------------
using System;
using System.Data;
...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class ShippingMethodStates : AspDotNetStorefront.SkinBase
	{
		...
		{
			if(DB.GetSqlN("select count(*) as N from State " + DB.GetNoLock()) == 0)
			{
			...
			else
			{
				...
				DataSet ds = DB.GetDS("select State.StateID,State.Name,ShippingMethodToStateMap.ShippingMethodID from State " + DB.GetNoLock() + " left outer join ShippingMethodToStateMap " + DB.GetNoLock() + " on State.StateID=ShippingMethodToStateMap.StateID and ShippingMethodToStateMap.ShippingMethodID=" + ShippingMethodID.ToString() + " order by displayorder,name",false);
				foreach(DataRow row in ds.Tables[0].Rows)
				{
					bool AllowedForThisState = DB.RowFieldInt(row,"ShippingMethodID") != 0;
					writer.Write("<tr bgcolor=\"" + AppLogic.AppConfig("LightCellColor") + "\">");
					writer.Write("<td>");
					writer.Write(DB.RowField(row,"Name"));
					writer.Write("</td>");
					writer.Write("<td>");
					writer.Write("<input type=\"checkbox\" name=\"StateList\" value=\"" + DB.RowFieldInt(row,"StateID").ToString() + "\" " + CommonLogic.IIF(AllowedForThisState, " checked ","") + ">");
					writer.Write("</td>");
					writer.Write("</tr>\n");
				}
				writer.Write("</table>");
