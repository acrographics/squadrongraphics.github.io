...
// $Header: /v6.2/Web/Admin/affiliates.aspx.cs 8     9/30/06 3:39p Redwoodtree $
// ------------------------------------------------------------------------------------------
using System;
using System.Data;
    ...
    public partial class Affiliates : System.Web.UI.Page
    {
        ...
        protected void resultFilter(string SearchFor)
        {
            ...
            if (SearchFor.Length == 0)
            {
                ...
                ThisCustomer.ThisCustomerSession.SetVal("AffiliatesSearch", txtSearch.Text);
            }
            ...
            if (Index.Equals("#"))
            {
                ...
                //WhereClause += " ([Company] like " + DB.SQuote("0%") + " or Name like " + DB.SQuote("0%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("1%") + " or Name like " + DB.SQuote("1%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("2%") + " or Name like " + DB.SQuote("2%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("3%") + " or Name like " + DB.SQuote("3%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("4%") + " or Name like " + DB.SQuote("4%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("5%") + " or Name like " + DB.SQuote("5%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("6%") + " or Name like " + DB.SQuote("6%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("7%") + " or Name like " + DB.SQuote("7%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("8%") + " or Name like " + DB.SQuote("8%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("9%") + " or Name like " + DB.SQuote("9%") + ")";
                //WhereClause += " OR ([Company] like " + DB.SQuote("10%") + " or Name like " + DB.SQuote("10%") + ")";
        ...
        {
            ltNotice.Text = "There are " + DB.GetSqlN("select count(affiliateid) as N from affiliate  " + DB.GetNoLock() + " where deleted=0").ToString() + " affiliates in the system.";
        ...
        private void GetAffiliates(DataTable dt, int ParentAffiliateID, int level)
        {
            ...
            DataSet ds = DB.GetDS("select AffiliateID, Name, EMail, URL, TrackingOnly from Affiliate  " + DB.GetNoLock() + " where parentAffiliateid=" + ParentAffiliateID.ToString() + " and deleted=0 order by DisplayOrder,Name", AppLogic.CachingOn, System.DateTime.Now.AddMinutes(AppLogic.CacheDurationMinutes()));
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                DataRow drow = dt.NewRow();
                drow[0] = row[0];
                drow[1] = Indent + row[1];
                drow[2] = row[2].ToString();
                drow[3] = row[3];
                drow[4] = row[4];
                dt.Rows.Add(drow);

                if ((DB.GetSqlN("select COUNT(*) AS N from Affiliate  " + DB.GetNoLock() + " where parentAffiliateid=" + ParentAffiliateID.ToString() + " and deleted=0")) > 0)
                {
                    GetAffiliates(dt, DB.RowFieldInt(row, "AffiliateID"), level + 1);
                }
            }
            ds.Dispose();
        }
        ...
        protected void gMain_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (Localization.ParseBoolean(myrow["TrackingOnly"].ToString()) || Localization.ParseNativeInt(myrow["TrackingOnly"].ToString()) == 1)
                {
                    ...
                    //ltName.Text = "<br/>(Ad Tracking Affiliate)";
